...
// import JoditEditor from "jodit-react";
  ...
  const Edit = ({ setActiveSection }) => {
    ...
    useEffect(() => {
      fetch(`${apiUrl}testimonials/${params.id}`, {
        method: "GET",
        headers: {
      ...
      .then(result => {
        ...
        setValue("testimonial", result.data.testimonial);
        setValue("status", result.data.status);
       
      })
      .catch(() => setError("Failed to fetch testimonial details"));
    }, [params.id, setValue]);
...
<div className="bg-white rounded-xl shadow-md p-6 mx-auto max-w-4xl mb-8">
    ...
    <h3 className="text-2xl font-semibold text-gray-800 mb-6">Update Testimonials</h3>
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Short Testimonial</label>
        <textarea 
          {...register("testimonial", { required: "testimonial is required" })} 
          className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all" 
          placeholder="Enter a short testimonial" 
          rows="3" 
        />
        {errors.testimonial && <p className="mt-1 text-sm text-red-500">{errors.testimonial.message}</p>}
      </div>
  ...
  <div className="space-y-3">
    ...
    <div>
      {selectedFile ? (
        ...
        testimonial.image && (
          <div>
            <p className="text-sm text-gray-600 mb-1">Current: {testimonial.image}</p>
            <img 
